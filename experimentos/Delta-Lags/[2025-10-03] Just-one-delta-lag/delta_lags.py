"""
Experimento Delta-Lags: Evaluación de features con delta 1 y lag 1
"""
...
def main(config_path, debug=None):
    ...
    n_deltas = exp_config.get('n_deltas', 1)  # Default para delta_lags
    n_lags = exp_config.get('n_lags', 1)  # Default para delta_lags
    ...
    delta_lag_transformer = DeltaLagTransformer(n_deltas=n_deltas, n_lags=n_lags)
    df = load_and_prepare_data(str(raw_data_path), str(target_data_path), delta_lag_transformer)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Ejecutar experimento Delta-Lags con configuración YAML')
