"""
Experimento Lags-Percentiles2: Evaluación de features con lag 2 y percentiles
"""
...
class LagsPercentilesTransformer:
    def __init__(self, n_lags=2, percentiles=[25, 50, 75, 90, 95]):
    ...
    def fit(self, X, y=None):
        ...
        self.percentile_transformer.fit(X)  # Los percentiles se calculan sobre datos originales
        return self
...
def main(config_path, debug=None):
    ...
    n_lags = exp_config.get('n_lags', 2)  # Default para lags_percentiles2
    percentiles = exp_config.get('percentiles', [25, 50, 75, 90, 95])  # Default percentiles
    ...
    lags_percentiles_transformer = LagsPercentilesTransformer(n_lags=n_lags, percentiles=percentiles)
    df = load_and_prepare_data(str(raw_data_path), str(target_data_path), lags_percentiles_transformer)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Ejecutar experimento Lags-Percentiles2 con configuración YAML')
